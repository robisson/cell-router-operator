...
func main() {
	...
	if err := (&controller.CellRouterReconciler{
		Client:   mgr.GetClient(),
		Scheme:   mgr.GetScheme(),
		Recorder: mgr.GetEventRecorderFor(constants.OperatorName + "-router"),
	}).SetupWithManager(mgr); err != nil {
		setupLog.Error(err, "unable to create controller", "controller", "CellRouter")
		os.Exit(1)
	}
